...
// > **Authentication:** The `PcRestoreSourceV2` resource does **not** support API key authentication. The CreateRestoreSource API routes requests to the target cluster (Prism Element), which only supports basic auth (username/password). Use `username` and `password` in the provider configuration—do not use `apiKey`. Using API key will result in "Invalid cookies present in the request" errors.
//
